...
static inline bool init_output(media_remux_job_t job, const char *out_filename)
{
	...
	for (unsigned i = 0; i < job->ifmt_ctx->nb_streams; i++) {
		...
		size_t content_size;
#endif
		const uint8_t *const content_src =
			av_stream_get_side_data(in_stream, AV_PKT_DATA_CONTENT_LIGHT_LEVEL, &content_size);
		if (content_src) {
			uint8_t *const content_dst =
				av_stream_new_side_data(out_stream, AV_PKT_DATA_CONTENT_LIGHT_LEVEL, content_size);
			if (content_dst)
...
#endif
		const uint8_t *const mastering_src =
			av_stream_get_side_data(in_stream, AV_PKT_DATA_MASTERING_DISPLAY_METADATA, &mastering_size);
		if (mastering_src) {
			uint8_t *const mastering_dst = av_stream_new_side_data(
				out_stream, AV_PKT_DATA_MASTERING_DISPLAY_METADATA, mastering_size);
			if (mastering_dst) {
				memcpy(mastering_dst, mastering_src, mastering_size);
			}
		}
#endif
		...
		if (in_stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
			...
			out_stream->codecpar->channel_layout =
				av_get_default_channel_layout(in_stream->codecpar->channels);
			...
			if (in_stream->codecpar->channels == 5)
				out_stream->codecpar->channel_layout = av_get_channel_layout("4.1");
#else
